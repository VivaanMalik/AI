...
float* Sigmoid::backward(float* d_grad, int batch_size, int feature_size) {
    ...
    if (batch_size != last_batch_size || total_size != current_size) {
        if (d_backward_result) cudaFree(d_backward_result);
...
ReLU::ReLU() : d_output(nullptr), d_backward_result(nullptr), d_mask(nullptr), current_size(0) {}
ReLU::~ReLU() {
    if (d_output) cudaFree(d_output);
...
float* ReLU::backward(float* d_grad, int batch_size, int feature_size) {
    ...
    int threads = 256;
    int blocks = (total_size + threads - 1) / threads;
    relu_backward_kernel<<<blocks, threads>>>(d_grad, d_backward_result, d_mask, total_size);
    // cudaDeviceSynchronize();
